...
class Board:
    def __init__(self, w, h, tile_size, tiles_images, lines_color, last_placed_color, default_color):
        # size
        self.w, self.h = w, h
    ...
    def change_tile_size(self, first):
        if first:
            ...
            print('Please write integer greater than 0')
        new_t_size = input()
        try:
            if int(new_t_size) <= 0:
                raise ValueError
    ...
    def change_size(self, first, show_incorrect_input=True):
        if first:
            print('\nWrite board width and height (cells)\nLike this: "20 30"\n This will create board 20 cells wide '
                  'and 30 cells in height')
        elif show_incorrect_input:
            print('Incorrect input!\ninput format: "width_in_cells height_in_cells" (2 integers)\nTry again...')
